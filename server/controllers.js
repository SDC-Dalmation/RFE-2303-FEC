...
module.exports = {
  getQuestions: (req, res) => {
    if (!req.params.product_id || req.params.product_id > 2147483647) {
  ...
  putAnswerHelpful: (req, res) => {
    if (
      parseInt(req.body.answer_id) === NaN ||
      req.body.answer_id > 2147483647
    ) {
  ...
  putQuestionReport: (req, res) => {
    if (
      parseInt(req.body.question_id) === NaN ||
      req.body.question_id > 2147483647
    ) {
      return res.status(400).send("invalid question id");
    }
